...
import { DoubleSide, LinearFilter, Vector2, PMREMGenerator, TextureLoader } from "three"
...
function Part() {
  ...
  useFrame(() => {
    ...
    billboard.current.rotation.x = Math.sin(Date.now() / speed) * 0.5
    ...
    billboard2.current.position.set(2.25, 1.5, 2.5)
  })
  ...
  const colorWheelMap = useLoader(TextureLoader, "color-wheel.png")
  const threeMap = useLoader(TextureLoader, "three.png")

  return (
    <object3D rotation={[0, Math.PI / 2, 0]}>
      <mesh ref={billboard} scale={[3, 3, 3]} dispose={null}>
        <planeBufferGeometry></planeBufferGeometry>
        <meshBasicMaterial map={colorWheelMap} transparent side={DoubleSide}></meshBasicMaterial>
      </mesh>
      <mesh ref={billboard2} scale={[3, 3, 3]} dispose={null}>
        <planeBufferGeometry></planeBufferGeometry>
        <meshBasicMaterial map={threeMap} transparent side={DoubleSide}></meshBasicMaterial>
      </mesh>
